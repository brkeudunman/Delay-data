...
# ============================================================
# 2. DATA LOADING (20% sample)
# ============================================================
print("=" * 60)
print("  STEP 1: Loading data (20% sample)")
print("=" * 60)
...
categorical_columns = [
    c for c in categorical_columns if c not in ("FL_YEAR", "FL_MONTH")
]

all_feature_cols = categorical_columns + continuous_columns
print(
    f"  Features: {len(categorical_columns)} categorical + {len(continuous_columns)} continuous"
)
...
importance_df = pd.DataFrame(
    {"Feature": all_feature_cols, "Mean|SHAP|": mean_abs_shap}
...
def build_utility_transactions(
    X_explain_df, X_explain_disc, shap_vals, mode="positive"
):
    """
...
pos_lines = build_utility_transactions(
    X_explain, X_explain_disc, shap_values, mode="positive"
)
neg_lines = build_utility_transactions(
    X_explain, X_explain_disc, shap_values, mode="negative"
)
...
def mine_patterns(input_file, label, min_util_percentile=MIN_UTIL_PCTILE):
    ...
    print(
        f"  [{label}] Transaction utility range: [{min(trans_utils)}, {max(trans_utils)}]"
    )
    ...
    for pattern_str, utility in patterns.items():
        ...
        item_ids = [
            int(x.strip()) for x in str(pattern_str).split("\t") if x.strip().isdigit()
        ]
...
print(
    f"  Delay protector patterns: {len(protectors_df) if not protectors_df.empty else 0}"
)
