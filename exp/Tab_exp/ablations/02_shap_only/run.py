...
02_shap_only/run.py
===================
Ablation: Top-N SHAP-selected features only. No HUIM patterns.

Run:
    python ablations/02_shap_only/run.py
"""
import gc
import os
import warnings
...
# ── Feature selection: top-N SHAP ─────────────────────────────────────────────
def select_shap_features(cat_cols, cont_cols):
    shap_df = pd.read_csv(SHAP_FILE)
    top_n   = FEAT_CFG["top_shap_n"]
    top     = shap_df.head(top_n)["Feature"].tolist()
    kept_cat  = [c for c in cat_cols  if c in top]
    kept_cont = [c for c in cont_cols if c in top]
    print(f"SHAP selection: {len(kept_cont)} continuous, {len(kept_cat)} categorical (top {top_n})")
...
def main():
    ...
    kept_cat, kept_cont = select_shap_features(cat_cols, cont_cols)
